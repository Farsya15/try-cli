...
# Should have 'user-myrepo' not 'myrepo.git' in the cd destination path
cd_line=$(echo "$output" | grep "cd '")
if echo "$cd_line" | grep -qE "/[0-9]{4}-[0-9]{2}-[0-9]{2}-user-myrepo'" && ! echo "$cd_line" | grep -q "myrepo.git"; then
    pass
else
    fail "clone should extract user-repo from URL" "cd to dated-user-myrepo (no .git)" "$cd_line" "command_line.md#clone"
fi
