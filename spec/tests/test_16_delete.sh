...
# Test: Delete script starts with cd to base dir
if echo "$output" | grep -q "^cd '"; then
    pass
else
    fail "Delete script should start with cd to base" "cd 'path'" "$output" "delete_spec.md#script-components"
fi
...
count=$(echo "$output" | grep -c "rm -rf" || true)
if [ "$count" -ge 2 ]; then
    pass
else
